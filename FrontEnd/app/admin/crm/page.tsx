          ...
          {recentContacts.map((contact) => (
            <div className={styles.leadRow} key={contact.id}>
              <div className={styles.leadMain}>
                <strong>
                  <Link href={`/admin/leads/${contact.id}`}>{contact.name}</Link>
                </strong>
                <span>{contact.email || contact.phone || "Sem contato informado"}</span>
              </div>
              <div className={styles.leadCell}>{contact.company || "Sem empresa"}</div>
              <div className={styles.leadCell}>
                <div>{contact.source_url || "Formulario"}</div>
                <small className={styles.statusInline}>
                  {contact.tracking_summary?.total_page_views ?? 0} paginas • ultima: {contact.tracking_summary?.last_page_accessed || "n/d"}
                </small>
              </div>
              <div className={styles.leadStatusWrap}>
                ...
                <span className={styles.statusInline}>
                  {getStatusLabel(contact.status)} • score {contact.lead_score ?? 0} ({contact.score_band || "cold"})
                </span>
              </div>
            </div>
          ))}
